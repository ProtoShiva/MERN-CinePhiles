...
export const bookMarkMovie = async (id) => {
  const token = getToken()
  try {
    const { data } = await client.put("/user/saveMovie", id, {
      headers: {
        authorization: "Bearer " + token
      }
    })
...
export const UnbookMarkMovie = async (id) => {
  const token = getToken()
  try {
    const { data } = await client.put("/user/unSaveMovie", id, {
      headers: {
        authorization: "Bearer " + token
      }
    })
